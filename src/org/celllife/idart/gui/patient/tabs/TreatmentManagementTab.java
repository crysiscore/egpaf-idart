...
public class TreatmentManagementTab extends GenericTab implements IPatientTab {
	...
	@Override
	public void clear() {
		btnNextAppointment.setText("Data da próxima consulata");
	...
	public void create() {
		...
		tabItem.setText("  Gestão de Tratamento  ");
	...
	private void createGrpTreatmentManagement() {
		...
		lblNextAppointment.setText("  Próxima consulta:");
		...
		btnNextAppointment.setText("Data Próxima consulta");
	...
	public Map<String, String> validateFields(Patient patient) {
		...
		if (btnNextAppointment.getDate() != null) {
			...
			if (!validNextAppointmentDate) {
				map.put("title", "Erro com a data da próxima consulta");
				map.put("message",
				" Não pode inserir a data da próxima consulta no passado. Seleccione uma data depois de hoje.");
			}
			return map;
		} else {
