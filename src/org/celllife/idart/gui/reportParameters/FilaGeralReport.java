...
import static org.celllife.idart.gui.platform.GenericReportGuiInterface.REPORT_FILA_GERAL;
...
import model.manager.reports.FilaGeralReporter;
/**
 */
public class FilaGeralReport extends GenericReportGui {
	 ...
	 *            boolean
	 */
	public FilaGeralReport(Shell parent, boolean activate) {
	...
	protected void createShell() {
		...
		buildShell(REPORT_FILA_GERAL, bounds);
	...
	protected void createCompHeader() {
		...
		buildCompdHeader(REPORT_FILA_GERAL, icoImage);
	}
	...
	protected void cmdViewReportWidgetSelected() {
		...
		if (iDARTUtil.before(calendarEnd.getCalendar().getTime(), calendarStart.getCalendar().getTime())){
			...
			try {
				...
				getLog()
				.error(
						"Exception while running FILA GERAL report",
						e);
			}
		}
