...
public abstract class GenericWelcome implements GenericGuiInterface {
	...
	private void createnewWelcome() {
		...
		//Cria tabela de Alerta de stock de ARVs
		
		//crateTableAlertARV();

		Composite compOptions = new Composite(shell, SWT.NONE);
		compOptions.setBounds(new Rectangle(145, 100, 610, 240));
	...
	private void crateTableAlertARV() {
		
	
		tblDrugs = new Table(shell, SWT.FULL_SELECTION);
		tblDrugs.setLinesVisible(true);
		tblDrugs.setBounds(new Rectangle(50, 400, 600, 200));
	...
	private void createCompHelpAndLogoff() {
		...
		compHelpAndLogoff.setBounds(new Rectangle(145, 220, 610, 115));
...
//				}
//				
//				if(lancar) new AlertLogOff(shell, false);;
			}
		});
	}
