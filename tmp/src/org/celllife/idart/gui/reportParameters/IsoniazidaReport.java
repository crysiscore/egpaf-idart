...
import model.manager.reports.IIsoniazidaReport;
...
public class IsoniazidaReport extends GenericReportGui {
	 ...
	 *            boolean
	 */
	public IsoniazidaReport(Shell parent, boolean activate) {
	...
	protected void createCompHeader() {
		...
		buildCompdHeader(REPORT_TPI, icoImage);
	}
	...
	protected void cmdViewReportWidgetSelected() {
		...
		if (iDARTUtil.before(calendarEnd.getCalendar().getTime(), calendarStart.getCalendar().getTime())){
			...
			try {
				...
				IIsoniazidaReport report = new IIsoniazidaReport(
						getShell(), theStartDate, theEndDate);
				viewReport(report);
			} catch (Exception e) {
				getLog()
				.error(
						"Exception while running Isoniazida report",
						e);
			}
		}
