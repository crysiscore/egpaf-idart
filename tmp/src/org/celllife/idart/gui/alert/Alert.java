...
public class Alert extends GenericFormGui  {
	...
	public Alert(Shell parent, Session hSession) {
 ...
 //Pendig Rupture --> stock <= AMC/3
				
				if(riscos.get(i).getAmc()/3>=riscos.get(i).getSaldo()){
					
					final TableItem item = new TableItem(tblDrugs, SWT.NONE);
				    item.setText(new String[] {""+(++k), riscos.get(i).getNome(), ""+riscos.get(i).getAmc(),""+riscos.get(i).getSaldo(), ""+riscos.get(i).getQtyOrdem(),"Roptura de Stock"});
				...
				if(riscos.get(i).getAmc()>=riscos.get(i).getSaldo()){
					...
					final TableItem item = new TableItem(tblDrugs, SWT.NONE);
				    item.setText(new String[] {""+(++k), riscos.get(i).getNome(), ""+riscos.get(i).getAmc(),""+riscos.get(i).getSaldo(), ""+riscos.get(i).getQtyOrdem(), "Stock Baixo"});
